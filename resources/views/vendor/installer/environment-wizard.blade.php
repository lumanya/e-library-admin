    ...
    <div class="tabs tabs-full">
        ...
        <form method="post" action="{{ route('LaravelInstaller::environmentSaveWizard') }}" class="tabs-wrap">
            <div class="tab" id="tab1content">
                ...
                <div class="buttons">
                    <button class="button next_setup">
            ...
            <div class="tab" id="tab3content">
                <div class="block">
                    ...
                    <div class="info">
                        ...
                        <div class="form-group {{ $errors->has('queue_driver') ? ' has-error ' : '' }}">
                            ...
                            <input type="text" name="queue_driver" id="queue_driver" value="database" placeholder="{{ trans('installer_messages.environment.wizard.form.app_tabs.queue_placeholder') }}" />
                ...
                <div class="block">
                    ...
                    <div class="info">
                        <div class="form-group {{ $errors->has('mail_driver') ? ' has-error ' : '' }}">
                            ...
                            <input type="text" name="mail_driver" id="mail_driver" value="smtp" placeholder="{{ trans('installer_messages.environment.wizard.form.app_tabs.mail_driver_placeholder') }}" readonly />
                        ...
                        <div class="form-group {{ $errors->has('mail_encryption') ? ' has-error ' : '' }}">
                            ...
                            <input type="text" name="mail_encryption" id="mail_encryption" value="tls" placeholder="{{ trans('installer_messages.environment.wizard.form.app_tabs.mail_encryption_placeholder') }}" />
                ...
                <div class="buttons">
                    ...
                    <button class="button d-none" id="install_setup" type="submit">
                        {{ trans('installer_messages.environment.wizard.form.buttons.install') }}
                        <i class="fa fa-angle-right fa-fw" aria-hidden="true"></i>
                    </button>
                </div>
...
@section('scripts')
    <script type="text/javascript">
        ...
        function checkValidation(type){
            ...
            switch (type) {
                case "mail" : data = {
                    ...
                    'environment_custom' : $("#environment_custom").val(),
                    'app_url' : $("#app_url").val(),
                    'app_log_level' : $("#app_log_level").val(),
                    'app_debug' : $("#app_debug").val(),
                    ...
                    'broadcast_driver' : $("#broadcast_driver").val(),
                    'cache_driver' : $("#cache_driver").val(),
                    'session_driver' : $("#session_driver").val(),
                    'database_port' : $("#database_port").val(),
                    'queue_driver' : $("#queue_driver").val(),
                ...
                case "database" : data = {
                    ...
                    break;
                default :  document.getElementById('#tab1').checked = true; $('#error_alert').remove(); return false; break;
            }
            ajaxCall(data);
        }
        ...
        function ajaxCall(data){
            $.ajax({
                ...
                success: function(result){
                    if(result.status){
                        if(result.tab == "tab4"){
                            $('#install_setup').trigger('click');
                        }else{
                            document.getElementById(result.tab).checked = true;
                            $('#error_alert').remove();
                        }
                    }else{
                        var error_message = `<div class="alert alert-danger" id="error_alert">
                ...
                error:function () {
                    ...
                    <ul>
                        <li>Mail Server Not Connected, Please check you mail configuration</li>
                                                </ul>
                                            </div>`;
                    $('#error_alert').remove();
                    $('.main').prepend(error_message);
        ...
        $(document).ready(function () {
            ...
            $(document).on('click','.next_setup',function () {
                var str = $(this).text();
                switch (str.trim()) {
                    case "Install" : checkValidation("mail");break;
                    case "Setup Database" : checkValidation("environment");break;
                    case "Setup Application" : checkValidation("database");break;
                    default :  document.getElementById('#tab1').checked = true; $('#error_alert').remove();break;
                }
                return false;
            });
